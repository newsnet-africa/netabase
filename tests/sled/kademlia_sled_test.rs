...
use crate::{
    DHT_PROPAGATION_WAIT, LONG_TIMEOUT, TEST_TIMEOUT, TestResults, current_timestamp_secs,
    init_debug_logger,
};
...
async fn test_kademlia_sled_persistence() {
    ...
    for i in 0..NUM_NODES {
        ...
        match Netabase::<SledKademliaSchema>::new(Some(db_path.clone())).await {
            Ok(netabase) => {
            ...
            Err(e) => {
                ...
                test_results.add_fail(format!("Failed to create node {}: {}", node_name, e));
                return;
            }
        }
    }
    ...
    for (node_name, netabase, _) in &nodes {
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                test_results.add_fail(format!("Peer connection failed for {}: {}", node_name, e));
            }
        }
    }

    if connected_nodes == 0 {
        error!("❌ No nodes successfully connected to peers");
    ...
    match test_sled_cross_node_data_sharing(&nodes, "sled_persistence_test").await {
        Ok((puts, retrievals, keys)) => {
            ...
            if puts > 0 {
                ...
                test_results.add_fail("No successful put operations".to_string());
            }

            if retrievals > 0 {
                test_results.add_pass();
            } else {
                test_results.add_fail("No successful retrieval operations".to_string());
            }

            // Additional verification: check all databases for expected keys
            ...
            for (node_name, _, db_path) in &nodes {
                match verify_sled_persistence(db_path, &keys).await {
                    ...
                    Ok(false) => {
                        ...
                        test_results
                            .add_fail(format!("Database verification failed for {}", node_name));
                    }
                    Err(e) => {
                        error!("❌ [{}] Database verification error: {}", node_name, e);
                        test_results.add_fail(format!(
                            "Database verification error for {}: {}",
                            node_name, e
                        ));
                    }
                }
        ...
        Err(e) => {
            ...
            test_results.add_fail(format!("Data sharing test failed: {}", e));
        }
    }
    ...
    match test_sled_provider_functionality(&nodes).await {
        Ok(successful_provisions) => {
            ...
            if successful_provisions > 0 {
                ...
                test_results.add_fail("No successful provider operations".to_string());
            }
        }
        Err(e) => {
            error!("❌ Sled provider test failed: {}", e);
            test_results.add_fail(format!("Provider test failed: {}", e));
        }
    }
        ...
        if let Some((_, _, db_path)) = nodes.first() {
            match verify_sled_persistence(db_path, &pre_restart_keys).await {
                ...
                Ok(false) => {
                    ...
                    test_results
                        .add_fail("Pre-restart persistence verification failed".to_string());
                }
                Err(e) => {
                    error!("❌ Pre-restart persistence verification error: {}", e);
                    test_results.add_fail(format!("Pre-restart persistence error: {}", e));
                }
            }
        }
    }
    ...
    // The temp_dirs will be automatically cleaned up when dropped
    info!(
    ...
    assert!(
        ...
        "No tests passed. Success rate: {:.1}%",
