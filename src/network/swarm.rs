...
#[derive(Debug, PartialEq, Eq)]
pub enum KadAction {
    Put { key: RecordKey, value: Vec<u8> },
    Get(RecordKey),
}
...
pub async fn handle_event(
    ...
    mut command_listener: tokio::sync::mpsc::Receiver<SwarmAction>,
) -> anyhow::Result<()> {
    init_logging();
    info!(
        "Is termination sent? {:?}",
        command_listener
            .recv()
            .await
            .eq(&Some(SwarmAction::EndLoop))
    );
    while Some(SwarmAction::EndLoop) != command_listener.recv().await {
        info!("Loop Starting");
        tokio::select! {
            event = swarm.select_next_some() => {
                ...
                libp2p::swarm::SwarmEvent::Behaviour(behaviour) => {
                    ...
                    match behaviour {
                        ...
                        crate::network::behaviour::NetabaseBehaviourEvent::Mdns(mdns) => {
                            info!("MDNS Event: {mdns:?}");match mdns {
                                libp2p::mdns::Event::Discovered(items) => {
                                    for (peer_id, mult) in &items {
                                        swarm.behaviour_mut().kad.add_address(&peer_id, mult.clone());
                                    }
                                    info!(
            ...
            command = command_listener.recv() => {
                info!("Received Command: {command:?}");
                match command {
                    Some(com) => match com {
                        SwarmAction::EndLoop => break,
